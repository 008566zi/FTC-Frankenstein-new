 ...
 *
 * <p>This OpMode shows the three required steps to add gamepad rumble to any TeleOp, plus
 * a clearly marked section where teammates can add custom one-liner conditions.</p>
 *
 * <p>Copy this pattern into any TeleOp; the three match-timer alerts are registered
 * automatically by the RumbleManager constructor.</p>
 */
...
public class RumbleDemoTeleOp extends OpMode {
    ...
    public void init() {
        ...
        // The three match-timer events (90 s, 105 s, 115 s) are registered automatically.
        rumble = new RumbleManager(gamepad1);

        // --- Add custom events here (fires once per false->true transition) ---
        // rumble.addEvent(() -> gamepad2.a,                                        () -> gamepad1.rumble(150));
        // rumble.addEvent(() -> robot.getIntakeStateName().equals("Holding"),       () -> gamepad1.rumbleBlips(1));
        // rumble.addEvent(() -> robot.getVisionStateName().equals("Tracking"),      () -> gamepad1.rumbleBlips(2));
    ...
    public void loop() {
        ...
        if (operatorInput.wasAJustPressed()) { robot.startIntake(); }
        else if (operatorInput.wasBJustPressed()) { robot.stopIntake(); }
        else if (operatorInput.wasXJustPressed()) { robot.ejectIntake(); }
        else if (operatorInput.wasYJustPressed()) { robot.holdIntake(); }
